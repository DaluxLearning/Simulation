!function() {
		...
		'<nav id="left_menu" class=\"navmenu navmenu-default navmenu-fixed-left offcanvas\">' +
			'<ul class=\"nav navmenu-nav\">' +	
				'<li id="navmenu_search_wrapper">' +
					...
					'</a>' +
				'</li>' +
				'<li id="navmenu_language_wrapper">' +
	...
	var buildDom = function(){
		...
		dom.courseTitle.text(contentApi.getProjectTitleForDisplay());
		dom.courseTitle.attr("title", DKI.utility.stripHTML(contentApi.getProjectTitleForDisplay()));
	...
	var initEvents = function(){
		...
		$(document).on(DKI.ContentPage.events.afterReady, function(e, page, pageContainer, contentPage){
				if(!$(contentPage.pageContainer).hasClass("dkiContentFrame") || !$(contentPage.pageContainer).hasClass("current")){
					return;
				}
				// processElements needs to be called after the other events.ready handlers
				setTimeout(function(){pageReady(e, contentPage)}, 1); // This timeout ensures that any elements that need to be set up after the page is ready (specifically tabsets) have been dealt with
			});
